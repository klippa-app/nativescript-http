...
import {
    getFilenameFromUrl,
    OkHTTPFormDataCommon,
    OkHTTPFormDataEntryCommon
} from "./okhttp-threaded.common";
...
let completeCallback: com.klippa.nativescriptokhttpthreaded.Async.CompleteCallback;
function ensureCompleteCallback() {
    if (completeCallback) {
        return;
    }

    completeCallback = new com.klippa.nativescriptokhttpthreaded.Async.CompleteCallback({
...
function onRequestComplete(requestId: number, result: com.klippa.nativescriptokhttpthreaded.Async.Http.RequestResult) {
    ...
    if (result.headers) {
        ...
        let pair: com.klippa.nativescriptokhttpthreaded.Async.Http.KeyValuePair;
...
function buildJavaOptions(options: HttpRequestOptions) {
    ...
    const javaOptions = new com.klippa.nativescriptokhttpthreaded.Async.Http.RequestOptions();
    ...
    if (options.content instanceof FormData || options.content instanceof OkHTTPFormData) {
        ...
        options.content.forEach(((value, key) => {
            ...
            } else if (value != null && typeof (value) !== "undefined" && !(value instanceof OkHTTPFormDataEntry)) {
            ...
            } else if (value instanceof OkHTTPFormDataEntry) {
    ...
    if (options.headers) {
        const arrayList = new java.util.ArrayList<com.klippa.nativescriptokhttpthreaded.Async.Http.KeyValuePair>();
        const pair = com.klippa.nativescriptokhttpthreaded.Async.Http.KeyValuePair;
...
export function request(options: HttpRequestOptions): Promise<HttpResponse> {
    ...
    return new Promise<HttpResponse>((resolve, reject) => {
        try {
            ...
            com.klippa.nativescriptokhttpthreaded.Async.Http.MakeRequest(javaOptions, completeCallback, new java.lang.Integer(requestIdCounter));
...
export class OkHTTPFormDataEntry extends OkHTTPFormDataEntryCommon {}
export class OkHTTPFormData extends OkHTTPFormDataCommon {}
